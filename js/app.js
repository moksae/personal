...
const App = (() => {
  ...
  function renderReviews() {
    ...
    list.innerHTML=filtered.map(r=>{
      ...
      const ab=S.isAdmin?'<div class="review-row-actions"><button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'reviews\',\''+r.id+'\')">'+(r.hidden?'◉':'○')+'</button><button class="act-btn" onclick="event.stopPropagation();App.editReview(\''+r.id+'\')">/</button><button class="act-btn del" onclick="event.stopPropagation();App.deleteReview(\''+r.id+'\')">×</button></div>':'';
  ...
  function openReview(id) {
    ...
    dv.innerHTML='<button class="td-back" onclick="App.closeReview()">← Back to reviews</button><div class="rd-header"><div class="rd-cat-badge"><span style="width:7px;height:7px;border-radius:50%;background:'+c.color+';display:inline-block"></span> '+c.emoji+' '+c.label+'</div><div class="rd-title">'+esc(r.title)+'</div>'+(rdTagsHTML?'<div class="writing-footer" style="margin-top:8px;margin-bottom:4px">'+rdTagsHTML+'</div>':'')+'<div class="rd-meta">'+fmtDate(r.timestamp)+'</div>'+ab+'</div><div class="rd-threads" style="--rd-cat-color:'+c.color+'">'+threadsHTML+'</div><div class="comments-wrap"><div id="review-comments"></div></div>';
  ...
  function renderWritings() {
    ...
    list.innerHTML=all.map(w=>{
      const ab=S.isAdmin?'<div class="writing-row-actions"><button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'writings\',\''+w.id+'\')">'+(w.hidden?'◉':'○')+'</button><button class="act-btn" onclick="event.stopPropagation();App.editWriting(\''+w.id+'\')">/</button><button class="act-btn del" onclick="event.stopPropagation();App.deleteWriting(\''+w.id+'\')">×</button></div>':'';
  ...
  function openWriting(id) {
    ...
    dv.innerHTML='<button class="td-back" onclick="App.closeWriting()">← Back to writings</button><div class="wd-header"><div class="wd-tags">'+((w.tags||[]).map(t=>'<span class="w-tag">'+esc(t)+'</span>').join(''))+'</div><div class="wd-title">'+esc(w.title)+'</div><div class="wd-meta">'+fmtDate(w.timestamp)+'</div>'+ab+'</div><div class="wd-body">'+fmtWriting(w.content)+'</div>'+(imgs?'<div class="post-images" style="padding:0 28px 20px">'+imgs+'</div>':'')+'<div class="comments-wrap"><div id="writing-comments"></div></div>';
