...
const App = (() => {
  ...
  function openFeedThread(threadId) {
    ...
    const ar=S.isAdmin?'<div class="td-actions"><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px" onclick="App.editPost(\''+root.id+'\')">/  edit</button><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px;color:var(--danger)" onclick="App.deletePost(\''+root.id+'\')">×  delete</button><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px" onclick="App.replyPost(\''+root.id+'\')">↩  reply</button></div>':'';
    const repliesHTML=rest.map(p=>{
      const ri=(p.images||[]).map(u=>'<img src="'+esc(u)+'" class="post-img" loading="lazy">').join('');
      const rA=S.isAdmin?'<div class="post-actions" style="opacity:1"><button class="act-btn" onclick="App.editPost(\''+p.id+'\')">/</button><button class="act-btn del" onclick="App.deletePost(\''+p.id+'\')">×</button><button class="act-btn" onclick="App.replyPost(\''+p.id+'\')">↩</button></div>':'';
  ...
  function openReview(id) {
    ...
    const c=CATS[r.category]||{label:r.category,emoji:'',color:'var(--text-dim)'};
    const ab=S.isAdmin?'<div class="wd-actions"><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px" onclick="App.editReview(\''+r.id+'\')">/  edit</button><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px;color:var(--danger)" onclick="App.deleteReview(\''+r.id+'\')">×  delete</button></div>':'';
  ...
  function openWriting(id) {
    ...
    const ab=S.isAdmin?'<div class="wd-actions"><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px" onclick="App.toggleHidden(\'writings\',\''+w.id+'\')">'+(w.hidden?'◉ show':'○ hide')+'</button><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px" onclick="App.editWriting(\''+w.id+'\')">/  edit</button><button class="btn btn-ghost" style="font-size:11px;padding:5px 12px;color:var(--danger)" onclick="App.deleteWriting(\''+w.id+'\')">×  delete</button></div>':'';
  ...
  function showLb() {
    ...
    if (ab) {
      ...
      if (S.isAdmin) {
        ...
        ab.innerHTML = '<button class="btn btn-ghost" style="font-size:11px;padding:4px 10px" onclick="App.editPic(\'' + p.id + '\')">edit</button><button class="btn btn-ghost" style="font-size:11px;padding:4px 10px" onclick="App.togglePicHidden(\'' + p.id + '\')">' + (p.hidden ? 'show' : 'hide') + '</button><button class="btn btn-ghost" style="font-size:11px;padding:4px 10px;color:var(--danger)" onclick="App.deletePic(\'' + p.id + '\')">del</button>';
      }
    }
  }
