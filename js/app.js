...
const App = (() => {
  ...
  function postCardHTML(root, isFirst) {
    ...
    const rootHTML  = singlePostHTML(root, true, noReplies);
    // When hasMore: show first + [collapsed middle] + last
    // When not hasMore: show all replies, last one gets isLast=true
    ...
    if (hasMore) {
      ...
      hidHTML   = '<div class="tw-more-wrap" id="more-'+tid+'">'
        + '<div class="tw-left"><div class="tw-vline"></div></div>'
        + '<div class="tw-more-content">'
        ...
        + middleReplies.map((p,i) => singlePostHTML(p, false, false)).join('')
        + '</div></div></div>';
    } else {
  ...
  function openReview(id) {
    ...
    dv.innerHTML='<button class="td-back" onclick="App.closeReview()">← Back to reviews</button><div class="rd-header"><div class="rd-cat-badge"><span style="width:7px;height:7px;border-radius:50%;background:'+c.color+';display:inline-block"></span> '+c.emoji+' '+c.label+'</div><div class="rd-title">'+esc(r.title)+'</div>'+(rdTagsHTML?'<div class="writing-footer" style="margin-top:8px;margin-bottom:4px">'+rdTagsHTML+'</div>':'')+'<div class="rd-meta">'+fmtDate(r.timestamp)+'</div>'+ab+'</div><div class="rd-threads" style="--rd-cat-color:'+c.color+'">'+threadsHTML+'</div><div class="emoji-reactions" id="reactions-review-'+r.id+'"></div>';
    renderReactions('review', r.id);
    window.scrollTo(0,0);
  }
  ...
  function openWriting(id) {
    ...
    dv.innerHTML='<button class="td-back" onclick="App.closeWriting()">← Back to writings</button><div class="wd-header"><div class="wd-tags">'+((w.tags||[]).map(t=>'<span class="w-tag">'+esc(t)+'</span>').join(''))+'</div><div class="wd-title">'+esc(w.title)+'</div><div class="wd-meta">'+fmtDate(w.timestamp)+'</div>'+ab+'</div><div class="wd-body">'+fmtWriting(w.content)+'</div>'+(imgs?'<div class="post-images" style="padding:0 28px 20px">'+imgs+'</div>':'')+'<div class="emoji-reactions" id="reactions-writing-'+w.id+'"></div>';
    renderReactions('writing', w.id);
    window.scrollTo(0,0);
  }
  ...
  function getReactions(type, id) {
    try { return JSON.parse(localStorage.getItem('rxn:'+type+':'+id)||'{}'); } catch { return {}; }
  }
  function hasReacted(type, id, emoji) {
    try { return JSON.parse(localStorage.getItem('rxn_me:'+type+':'+id)||'[]').includes(emoji); } catch { return false; }
  }
  function toggleReaction(type, id, emoji) {
    const data = getReactions(type, id);
    const me = JSON.parse(localStorage.getItem('rxn_me:'+type+':'+id)||'[]');
    if (me.includes(emoji)) {
      data[emoji] = Math.max(0, (data[emoji]||1) - 1);
      me.splice(me.indexOf(emoji), 1);
    } else {
      data[emoji] = (data[emoji]||0) + 1;
      me.push(emoji);
    }
    localStorage.setItem('rxn:'+type+':'+id, JSON.stringify(data));
    localStorage.setItem('rxn_me:'+type+':'+id, JSON.stringify(me));
    renderReactions(type, id);
  }
  function renderReactions(type, id) {
    const el = ge('reactions-'+type+'-'+id);
    if (!el) return;
    const data = getReactions(type, id);
    el.innerHTML = '<div class="rxn-bar">'
      + REACTION_EMOJIS.map(e => {
          const cnt = data[e]||0;
          const active = hasReacted(type, id, e);
          return '<button class="rxn-btn'+(active?' rxn-active':'')+'" onclick="App.toggleReaction(\''+type+'\',\''+id+'\',\''+e+'\')">'
            + e + (cnt > 0 ? '<span class="rxn-count">'+cnt+'</span>' : '')
            + '</button>';
        }).join('')
      + '</div>';
  }
  ...
  function renderPics() {
    ...
    const start = S.picsPage * PICS_PER_PAGE;
    const pagePics = pics.slice(start, start + PICS_PER_PAGE);
    if (!pics.length) {
  ...
  function showLb() {
    ...
    // render reactions for this pic
    const rxnEl = ge('reactions-pic-current');
    if (rxnEl) rxnEl.id = 'reactions-pic-'+p.id;
    setTimeout(() => renderReactions('pic', p.id), 50);
  ...
  return {
    ...
    toggleReaction, renderReactions,
