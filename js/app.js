...
const App = (() => {
  ...
  // Strip invisible unicode / non-ASCII that cause setRequestHeader to throw
  function ascii(s) { return (s||'').replace(/[^\x20-\x7E]/g,'').trim(); }

  // Use HTTP Basic Auth: btoa output is PURE ASCII → header error is impossible
  function makeAuth() {
    return 'Basic ' + btoa(ascii(S.owner) + ':' + ascii(S.token));
  }

  function ghXHR(method, path, body) {
    return new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open(method, 'https://api.github.com/repos/' + ascii(S.owner) + '/' + ascii(S.repo) + path, true);
      xhr.setRequestHeader('Authorization', makeAuth());
  ...
  // ── Admin ──────────────────────────────────────
  function loadCreds() {
    const t=ascii(localStorage.getItem('gh_token'));
    const o=ascii(localStorage.getItem('gh_owner'));
    const r=ascii(localStorage.getItem('gh_repo'));
