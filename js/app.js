...
const App = (() => {
  ...
  function postCardHTML(root, isFirst) {
    ...
    if (hasMore) {
      ...
      // "more" row has the same structure as a tw-post: left-col (vline only) + right-col (button)
  ...
  function reviewCardHTML(r) {
    ...
    const ab = S.isAdmin ? hidBtn+'<button class="act-btn" onclick="event.stopPropagation();App.editReview(\''+r.id+'\')">/</button><button class="act-btn del" onclick="event.stopPropagation();App.deleteReview(\''+r.id+'\')">×</button>' : '';
  ...
  function writingCardHTML(w) {
    ...
    const ab = S.isAdmin ? hidBtn+'<button class="act-btn" onclick="event.stopPropagation();App.editWriting(\''+w.id+'\')">/</button><button class="act-btn del" onclick="event.stopPropagation();App.deleteWriting(\''+w.id+'\')">×</button>' : '';
  ...
  function toggleThreadMore(tid) {
    ...
    const btn = document.querySelector('#more-'+tid+' > .tw-right > .tw-more-btn');
    if (!body) return;
  ...
  function getPicsPerPage() { return window.innerWidth <= 760 ? 6 : 8; }
  ...
  function bindGlobal() {
    ...
    // Scroll top / bottom
    bindClick('scroll-top-btn', () => { const m=document.querySelector('.main'); if(m) m.scrollTo({top:0,behavior:'smooth'}); });
    bindClick('scroll-bot-btn', () => { const m=document.querySelector('.main'); if(m) m.scrollTo({top:m.scrollHeight,behavior:'smooth'}); });
  ...
  return {
    ...
    openMobileNav, closeMobileNav, mobileNav, editPic,
