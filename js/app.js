...
const App = (() => {
  ...
  // FEED_THREAD_COLLAPSE: replies shown inline, collapse after this many
  const THREAD_PREVIEW = 2;

  function postCardHTML(root) {
    ...
    function singlePostHTML(p, isRoot) {
      ...
      const meta = '<div class="post-meta">'
        ...
        + '</div>';
      return '<div class="ft-post'+(isRoot?' ft-root':' ft-reply')+'">'
        + (isRoot ? '' : '<div class="ft-connector"><div class="ft-line"></div><div class="ft-dot"></div></div>')
    ...
    const hasMore = allReplies.length > THREAD_PREVIEW;
    const visibleReplies = hasMore ? allReplies.slice(0, THREAD_PREVIEW) : allReplies;
    const hiddenReplies = hasMore ? allReplies.slice(THREAD_PREVIEW) : [];

    const visHTML = visibleReplies.map(p => singlePostHTML(p, false)).join('');
    const hidHTML = hiddenReplies.length
      ? '<div class="ft-more-wrap" id="more-'+tid+'">'
        + '<button class="ft-more-btn" onclick="event.stopPropagation();App.toggleThreadMore(\''+tid+'\')">↓ '+hiddenReplies.length+'개 더 보기</button>'
        + '<div class="ft-hidden-replies" id="more-body-'+tid+'" style="display:none">'
        + hiddenReplies.map(p => singlePostHTML(p, false)).join('')
        + '</div></div>'
      : '';

    return '<div class="post-wrap ft-thread'+(root.hidden?' is-hidden':'')+'" onclick="App.openFeedThread(\''+tid+'\')">'
      + rootHTML + visHTML + hidHTML
      + '</div>';
  }
