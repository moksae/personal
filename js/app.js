...
const App = (() => {
  ...
  function fmtWriting(raw) {
    ...
    return blocks.map(block => {
      ...
      // Heading: ## text
      if (/^##\s/.test(t)) {
        return '<h3 class="wd-heading">' + esc(t.replace(/^##\s+/,'')) + '</h3>';
      }
      // Divider: --- alone
      if (/^-{3,}$/.test(t)) {
        return '<hr class="wd-hr">';
      }
      // Blockquote: > lines
      if (/^>/.test(t)) {
        const inner = t.split('\n').map(l => esc(l.replace(/^>\s?/,''))).join('<br>');
        return '<blockquote class="wd-quote">' + inner + '</blockquote>';
      }
      // Inline image: [img:url]
      if (/^\[img:.+\]$/.test(t)) {
        const url = t.match(/^\[img:(.+)\]$/)[1];
        return '<div class="wd-inline-img"><img src="' + esc(url) + '" loading="lazy"></div>';
      }
      // Normal paragraph — also handle inline [img:url] within text
      const para = esc(t)
        .replace(/\n/g, '<br>')
        .replace(/\[img:([^\]]+)\]/g, '<div class="wd-inline-img"><img src="$1" loading="lazy"></div>');
      return '<p>' + para + '</p>';
    }).join('');
  }
  let _tt;
  ...
  function postCardHTML(root, isFirst) {
    ...
    function singlePostHTML(p, isRoot, isLast) {
      const imgs = (p.images||[]).map(u => '<img src="'+esc(u)+'" class="post-img" loading="lazy">').join('');
      const ab = S.isAdmin
        ? '<button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'posts\',\''+p.id+'\')">'+(p.hidden?'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button>'
        + '<button class="act-btn" onclick="event.stopPropagation();App.editPost(\''+p.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button>'
        + '<button class="act-btn del" onclick="event.stopPropagation();App.deletePost(\''+p.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>'
        + '<button class="act-btn" onclick="event.stopPropagation();App.replyPost(\''+p.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 14 4 9 9 4"/><path d="M20 20v-7a4 4 0 0 0-4-4H4"/></svg></button>'
        : '';
      ...
      const bulkCb = (_bulkMode && S.isAdmin) ? '<input type="checkbox" class="bulk-cb" onchange="App.toggleBulkSelect(\''+p.id+'\')" onclick="event.stopPropagation()" '+((_bulkSelected.has(p.id))?'checked':'')+' style="margin-right:6px;accent-color:var(--accent)">' : '';
    return '<div class="tw-post'+(isRoot?' tw-root':' tw-reply')+'" data-bulk-id="'+p.id+'"'+(_bulkMode&&S.isAdmin?' onclick="event.stopPropagation();App.toggleBulkSelect(\''+p.id+'\')"':'')+'>'
        + '<div class="tw-left">'
    ...
    // Render: root always shows vline if there are any replies
    // All replies show vline except the very last one in the thread
    const allPosts = [root, ...allReplies];
    const rootHTML = singlePostHTML(root, true, allReplies.length === 0);

    let firstHTML='', hidHTML='', lastHTML='';
    if (hasMore) {
      firstHTML = firstReply.map(p => singlePostHTML(p, false, false)).join('');
      // collapsed button — same horizontal level as posts, no indentation
      // "more" button row — same structure as tw-post (dot col + content col)
      // Hidden replies are SIBLINGS (not nested), so they render at same level
      hidHTML = '<div class="tw-more-item" id="more-'+tid+'">'
        + '<div class="tw-left"><div class="tw-vline-only"></div></div>'
        + '<div class="tw-right">'
        ...
        + middleReplies.map(p => singlePostHTML(p, false, false)).join('')
        + '</div>';
      lastHTML = lastReply.map(p => singlePostHTML(p, false, true)).join('');
    } else {
      firstHTML = allReplies.map((p,i) => singlePostHTML(p, false, i===allReplies.length-1)).join('');
    }
  ...
  function reviewCardHTML(r) {
    ...
    const hidBtn = S.isAdmin ? '<button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'reviews\',\''+r.id+'\')">'+(r.hidden?'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button>' : '';
    const ab = S.isAdmin ? hidBtn+'<button class="act-btn" onclick="event.stopPropagation();App.editReview(\''+r.id+'\'"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="event.stopPropagation();App.deleteReview(\''+r.id+'\'"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>' : '';
  ...
  function writingCardHTML(w) {
    const hidBtn = S.isAdmin ? '<button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'writings\',\''+w.id+'\')">'+(w.hidden?'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button>' : '';
    const ab = S.isAdmin ? hidBtn+'<button class="act-btn" onclick="event.stopPropagation();App.editWriting(\''+w.id+'\'"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="event.stopPropagation();App.deleteWriting(\''+w.id+'\'"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>' : '';
  ...
  function openFeedThread(threadId) {
    ...
    const ar=S.isAdmin?'<div class="td-actions">' + (S.isAdmin ? '<button class="act-btn" onclick="App.editPost(\''+root.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="App.deletePost(\''+root.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button><button class="act-btn" onclick="App.replyPost(\''+root.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 14 4 9 9 4"/><path d="M20 20v-7a4 4 0 0 0-4-4H4"/></svg></button>' : '') + '</div>':'';
    const repliesHTML=rest.map(p=>{
      const ri=(p.images||[]).map(u=>'<img src="'+esc(u)+'" class="post-img" loading="lazy">').join('');
      const rA=S.isAdmin?'<div class="post-actions"><button class="act-btn" onclick="App.editPost(\''+p.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="App.deletePost(\''+p.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button><button class="act-btn" onclick="App.replyPost(\''+p.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 14 4 9 9 4"/><path d="M20 20v-7a4 4 0 0 0-4-4H4"/></svg></button></div>':'';
    ...
    if (tp) tp.innerHTML='<div class="td-body"><div class="td-root"><div style="margin-bottom:14px"><div style="font-weight:700;font-size:14px">'+esc(S.data.config.author||'Author')+'</div><div style="font-size:11px;color:var(--text-sub);margin-top:2px">'+relTime(root.timestamp)+'</div></div><div class="td-content">'+fmtContent(root.content)+'</div>'+(rootImgs?'<div class="post-images">'+rootImgs+'</div>':'')+'<div class="td-meta">'+fmtDate(root.timestamp)+'</div>'+ar+'</div>'+repliesHTML+'</div>';
    // no Giscus on main feed
    window.scrollTo(0,0);
  }
  function toggleThreadMore(tid) {
    const body = ge('more-body-'+tid);
    const btn = document.querySelector('#more-'+tid+' .tw-more-btn');
    if (!body) return;
    const open = body.style.display !== 'none';
    body.style.display = open ? 'none' : 'block';
    const count = body.querySelectorAll('.tw-post').length;
    if (btn) btn.textContent = open ? '↓ ' + count + '개 더 보기' : '↑ 접기';
  }
  ...
  function renderReviews() {
    ...
    list.innerHTML=filtered.map(r=>{
      ...
      const ab=S.isAdmin?'<div class="review-row-actions"><button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'reviews\',\''+r.id+'\')">'+(r.hidden?'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button><button class="act-btn" onclick="event.stopPropagation();App.editReview(\''+r.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="event.stopPropagation();App.deleteReview(\''+r.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button></div>':'';
  ...
  function openReview(id) {
    ...
    const c=CATS[r.category]||{label:r.category,emoji:'',color:'var(--text-dim)'};
    const ab=S.isAdmin?'<div class="wd-actions"><button class="act-btn" onclick="App.toggleHidden(\'reviews\',\''+r.id+'\')">'+(r.hidden?'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button><button class="act-btn" onclick="App.editReview(\''+r.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="App.deleteReview(\''+r.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button></div>':'';
    ...
    dv.innerHTML='<button class="td-back" onclick="App.closeReview()">← Back to reviews</button><div class="rd-header"><div class="rd-cat-badge"><span style="width:7px;height:7px;border-radius:50%;background:'+c.color+';display:inline-block"></span> '+c.emoji+' '+c.label+'</div><div class="rd-title">'+esc(r.title)+'</div>'+(rdTagsHTML?'<div class="writing-footer" style="margin-top:8px;margin-bottom:4px">'+rdTagsHTML+'</div>':'')+'<div class="rd-meta">'+fmtDate(r.timestamp)+'</div>'+ab+'</div><div class="rd-threads" style="--rd-cat-color:'+c.color+'">'+threadsHTML+'</div>';
    window.scrollTo(0,0);
  }
  ...
  function renderWritings() {
    ...
    list.innerHTML=all.map(w=>{
      const ab=S.isAdmin?'<div class="writing-row-actions"><button class="act-btn" onclick="event.stopPropagation();App.toggleHidden(\'writings\',\''+w.id+'\')">'+(w.hidden?'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button><button class="act-btn" onclick="event.stopPropagation();App.editWriting(\''+w.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="event.stopPropagation();App.deleteWriting(\''+w.id+'\')"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button></div>':'';
  ...
  function openWriting(id) {
    ...
    const ab=S.isAdmin?'<div class="wd-actions"><button class="act-btn" onclick="App.toggleHidden(\'writings\',\''+w.id+'\')">'+(w.hidden?'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button><button class="act-btn" onclick="App.editWriting(\''+w.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn del" onclick="App.deleteWriting(\''+w.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button></div>':'';
    dv.innerHTML='<button class="td-back" onclick="App.closeWriting()">← Back to writings</button><div class="wd-header"><div class="wd-tags">'+((w.tags||[]).map(t=>'<span class="w-tag">'+esc(t)+'</span>').join(''))+'</div><div class="wd-title">'+esc(w.title)+'</div><div class="wd-meta">'+fmtDate(w.timestamp)+'</div>'+ab+'</div><div class="wd-body">'+fmtWriting(w.content)+'</div>'+(imgs?'<div class="post-images" style="padding:0 28px 20px">'+imgs+'</div>':'')+'';
    window.scrollTo(0,0);
  }
  ...
  function openWritingCompose(isEdit) {
    ...
    if (!isEdit) {
      ...
      if (ge('wc-excerpt')) ge('wc-excerpt').value='';
      if (ge('wc-content')) ge('wc-content').value='';
      if (ge('wc-char-num')) ge('wc-char-num').textContent='0';
  ...
  async function sendWriting() {
    ...
    const excerptInput=(ge('wc-excerpt')?ge('wc-excerpt').value.trim():'');
    if (!title||!content) { toast('Title and content required','err'); return; }
    const tags=tagsRaw?tagsRaw.split(',').map(t=>t.trim()).filter(Boolean):[];
    const excerpt=excerptInput||content.replace(/#{1,3}\s|>\s|---\n?|\[img:[^\]]+\]/g,'').substring(0,80)+(content.length>80?'...':'');
  ...
  function editWriting(id) {
    ...
    if(ge('wc-excerpt')) ge('wc-excerpt').value=w.excerpt||'';
  ...
  function openMobileNav() {
    ...
    const drawer  = ge('mobile-sidebar');
  ...
  function toggleMobileNav() {
    const drawer = ge('mobile-sidebar');
    if (drawer && drawer.classList.contains('open')) closeMobileNav();
    else openMobileNav();
  }
  ...
  function updateAdminUI() {
    ...
    const btb = ge('bulk-toggle-btn');
    if(S.isAdmin){
      if(btn){btn.classList.add('on');const sp=btn.querySelector('.al');if(sp)sp.textContent='Logout';}
      if(fab)fab.classList.add('show');
      if(mab)mab.innerHTML='<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg> Logout';
      ...
      if(mab)mab.innerHTML='<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93l-1.41 1.41M5.34 18.66l-1.41 1.41M22 12h-2M4 12H2M19.07 19.07l-1.41-1.41M5.34 5.34L3.93 3.93"/><circle cx="12" cy="12" r="9" stroke-width="1.4"/></svg> Admin';
  ...
  function toggleBulkMode() {
    _bulkMode = !_bulkMode;
    _bulkSelected.clear();
    const bar = ge('bulk-bar');
    const btn = ge('bulk-toggle-btn');
    if (bar) bar.style.display = _bulkMode ? 'flex' : 'none';
    if (btn) btn.textContent = _bulkMode ? 'Done' : 'Select';
    renderHome();
  }
  function toggleBulkSelect(id) {
    if (_bulkSelected.has(id)) _bulkSelected.delete(id);
    else _bulkSelected.add(id);
    const el = document.querySelector('[data-bulk-id="'+id+'"]');
    if (el) el.classList.toggle('bulk-selected', _bulkSelected.has(id));
    ge('bulk-count') && (ge('bulk-count').textContent = _bulkSelected.size + ' selected');
  }
  async function bulkHide() {
    if (!_bulkSelected.size) return;
    S.data.posts.forEach(p => { if (_bulkSelected.has(p.id)) p.hidden = true; });
    await savePostsData('Bulk hide');
    _bulkSelected.clear(); renderHome(); toast('Hidden ✓','ok');
  }
  async function bulkDelete() {
    if (!_bulkSelected.size || !confirm('Delete '+_bulkSelected.size+' posts?')) return;
    S.data.posts = S.data.posts.filter(p => !_bulkSelected.has(p.id));
    await savePostsData('Bulk delete');
    _bulkSelected.clear(); renderHome(); toast('Deleted','ok');
  }
  ...
  function getPicsPerPage() { return window.innerWidth <= 760 ? 6 : 12; }
  if (!S.picsPage) S.picsPage = 0;
  function renderPics() {
    const pics = (S.data.pics||[]).filter(p => S.isAdmin || !p.hidden);
    const grid = ge('pics-grid');
    if (!grid) return;
    const PPP = getPicsPerPage();
    const totalPages = Math.ceil(pics.length / PPP);
    if (S.picsPage >= totalPages) S.picsPage = Math.max(0, totalPages-1);
    const start = S.picsPage * PPP;
    const pagePics = pics.slice(start, start + PPP);
    if (!pics.length) {
      grid.innerHTML = '<div class="loading-state" style="padding:40px;text-align:center;color:var(--text-dim)">No pics yet</div>';
    } else {
      grid.innerHTML = pagePics.map((p, i) => {
        const src = (p.images||[])[0] || '';
        const hb = p.hidden ? '<span class="hidden-badge">hidden</span>' : '';
        return '<div class="pic-item" onclick="App.openLightbox('+(start+i)+')"><img src="'+esc(src)+'" loading="lazy" alt="'+esc(p.caption||'')+'">'+hb+'</div>';
      }).join('');
      // pagination controls
      const nav = ge('pics-page-nav');
      if (nav) {
        nav.innerHTML = totalPages > 1
          ? '<button class="page-btn" onclick="App.picsPageNav(-1)" '+(S.picsPage===0?'disabled':'')+'>← Prev</button>'
          + '<span style="font-size:12px;font-family:var(--font-mono);color:var(--text-dim)">'+(S.picsPage+1)+' / '+totalPages+'</span>'
          + '<button class="page-btn" onclick="App.picsPageNav(1)" '+(S.picsPage>=totalPages-1?'disabled':'')+'>Next →</button>'
          : '';
      }
    }
  ...
  function picsPageNav(dir) { if(!S.picsPage) S.picsPage=0; S.picsPage+=dir; renderPics(); }
  ...
  function showLb() {
    ...
    if (ab) {
      ...
      if (S.isAdmin) {
        ...
        ab.innerHTML = '<button class="act-btn" onclick="App.editPic(\''+p.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4z"/></svg></button><button class="act-btn" onclick="App.togglePicHidden(\''+p.id+'\')">'+(p.hidden?'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 9.9-1"/></svg>':'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>')+'</button><button class="act-btn del" onclick="App.deletePic(\''+p.id+'\')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg></button>';
      }
    }
  }
  ...
  async function editPic(id) {
    ...
    const newTagsStr = prompt('Tags (comma-separated):', (p.tags||[]).join(', '));
    if (newTagsStr === null) return;
    p.caption = newCaption.trim();
    p.tags = newTagsStr.split(',').map(t=>t.trim()).filter(Boolean);
    await savePicsData('Edit pic');
    showLb(); renderPics();
    toast('Updated ✓', 'ok');
  }
  ...
  function bindGlobal() {
    ...
    bindClick('nav-reviews',  ()=>pushNav('reviews'));
    ...
    // Writing char counter
    const wta = ge('wc-content');
    if (wta) {
      const updateCount = () => { const n = ge('wc-char-num'); if (n) n.textContent = wta.value.length.toLocaleString(); };
      wta.addEventListener('input', updateCount);
    }
    ...
    // Word count for wc-content
    document.addEventListener('input', e => {
      if (e.target.id === 'wc-content') {
        const wc = ge('wc-wordcount');
        if (!wc) return;
    ...
    bindClick('scroll-bot-btn', () => window.scrollTo({top:document.documentElement.scrollHeight,behavior:'smooth'}));
  ...
  return {
    ...
    openMobileNav, closeMobileNav, toggleMobileNav, mobileNav, editPic,
    picsPageNav, toggleBulkMode, toggleBulkSelect, bulkHide, bulkDelete,
