...
const App = (() => {
  ...
  // ── Giscus Comments ───────────────────────────
  // innerHTML로 <script> 삽입하면 브라우저가 실행 안 함 → createElement 사용
  ...
  function renderProfile() {
    ...
    if (ge('stat-posts'))    ge('stat-posts').textContent    = S.data.posts.filter(p=>!p.parentId).length + S.data.reviews.length + S.data.writings.length;
  ...
  function renderCounts() {
    const total = S.data.posts.filter(p=>!p.parentId).length + S.data.reviews.length + S.data.writings.length;
    if (ge('badge-home'))     ge('badge-home').textContent     = S.data.posts.filter(p=>!p.parentId).length + S.data.reviews.length + S.data.writings.length;
  ...
  function renderSection(sec) {
    ...
    const pageId = { home:'page-home', reviews:'page-reviews', writings:'page-writings', guestbook:'page-guestbook' }[sec];
  ...
  function openReview(id) {
    ...
    dv.innerHTML='<button class="td-back" onclick="App.closeReview()">← Back to reviews</button><div class="rd-header"><div class="rd-cat-badge"><span style="width:7px;height:7px;border-radius:50%;background:'+c.color+';display:inline-block"></span> '+c.emoji+' '+c.label+'</div><div class="rd-title">'+esc(r.title)+'</div><div class="rd-meta">'+fmtDate(r.timestamp)+(r.edited?' · edited':'')+'</div>'+ab+'</div><div class="rd-threads">'+threadsHTML+'</div><div class="comments-wrap"><div id="review-comments"></div></div>';
